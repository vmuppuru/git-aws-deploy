...
//optional params
function display1(name : string, id : number, gender ?: string): any{
    console.log(name +" "+id +" "+gender);
}
display1("vani",101);

//default values for params
 function show(name : string, gender : string, id : number=100): any{
    console.log(name +" "+id +" "+gender);
}
show("vani",  "female");
...
function add(num1 : number, num2: number): number{
 ...
 calc(add);
 //exercise
 var report = (maths : number, physics : number, chemistry : number)=>{
    ...
    if(avg>90){
        ...
        else if(avg>=35 && avg<50){console.log("pass")} 
